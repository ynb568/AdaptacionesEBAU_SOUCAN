...
EXEC sp_registraCentroEducativo 
    ...
    @contrasenha = 'contrase�aPrueba', 
    @repetirContrasenha = 'contrase�aPrueba',
    @Mensaje = '', 
    @Registrado = 0
go
					...
					begin
						if (not exists (select * from Estudiante where dniEstudiante = @dniE))
							begin
								if (exists (select * from PlazosRegistro where activo = 1))
									begin
										...
										set @idE = SCOPE_IDENTITY(); --OBTIENE LA PK DEL �LTIMO REGISTRO INSERTADO
							...
							begin
								if @@trancount > 0
								rollback transaction
								set @Mensaje = 'El dni del estudiante est� duplicado';
								set @Registrado = 0;
							end
					end
				else
